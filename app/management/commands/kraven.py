...
class Command(BaseCommand):
    ...
    help = "Play around with Kraven Application!"
    ...
    def handle(self, *args, **options):
        ...
        if len(options['command']) == 0 or options['command'][0] not in self.available:
            raise CommandError('Command Does not exist! Please use one of the following: python manage.py kraven [%s]' % ", ".join(self.available))
